...
class Mech extends CI_Controller{
    ...
    public function dashboard(){
        if ($this->session->userdata('user_login_access') != False) {
            ...
            $data['todolist'] = $this->mech_model->getTodoInfo($userid);
            $this->load->view('mech/dashboard', $data);
        } else {
    ...
    public function List_user(){
        if ($this->session->userdata('user_login_access') != False) {
            $data                  = array();
            $data['userlist']      = $this->mech_model->getAllUsers();
            $data['settingsvalue'] = $this->mech_model->getSettingsValue();
            $this->load->view('mech/userlist', $data);
        } else {
            redirect(base_url(), 'refresh');
        }
    }
    ...
    public function List_user_updated(){
        if ($this->session->userdata('user_login_access') != False) {
            ...
            $data['userlist']      = $this->mech_model->getAllUsers();
            $data['settingsvalue'] = $this->mech_model->getSettingsValue();
            $this->load->view('mech/userlist-updated', $data);
        } else {
            redirect(base_url(), 'refresh');
        }
    }
    ...
    public function Add_User(){
        if ($this->session->userdata('user_login_access') != False) {
            $data['settingsvalue'] = $this->mech_model->getSettingsValue();
            $this->load->view('mech/adduser', $data);
        } else {
            redirect(base_url(), 'refresh');
        }
    }
    ...
    public function user_delet(){
        if ($this->session->userdata('user_login_access') != False) {
            ...
            $this->mech_model->userTableDelet($id);
            if ($this->db->affected_rows()) {
                $profile    = $this->mech_model->getUserValue($id);
                $checkimage = "./assets/img/user/$profile->image";
                if (file_exists($checkimage)) {
                    unlink($checkimage);
                    redirect('crud/List_user');
                }
                /*      $this->mech_model->User_Notes_Delet($id);
                $this->mech_model->User_commentid_Delet($id);*/
                
            } else {
                redirect('mech/List_user', 'refresh');
            }
        }
    }
    /*user profile*/
    public function View_profile(){
        if ($this->session->userdata('user_login_access') != False) {
            ...
            $data                  = array();
            $data['settingsvalue'] = $this->mech_model->getSettingsValue();
            $data['profile']       = $this->mech_model->getProfileValue($userid);
            $data['usernote']      = $this->mech_model->getUserNotes($userid);
            $this->load->view('mech/profile', $data);
        } else {
            redirect(base_url(), 'refresh');
        }
    }
    ...
    public function viewUserDataBYID(){
        if ($this->session->userdata('user_login_access') != False) {
            $id                = $this->input->get('id');
            $data['uservalue'] = $this->mech_model->getUserValue($id);
                ...
                if ($this->login_model->Does_email_exists($email)) {
                    ...
                    if ($_FILES['user_image']['name']) {
                        ...
                        if (!$this->upload->do_upload('user_image')) {
                            ...
                            $success             = $this->mech_model->addUserInfo($data);
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if ($_FILES['user_image']['name']) {
                    ...
                    if (!$this->upload->do_upload('user_image')) {
                        ...
                        $success             = $this->mech_model->UserUpdate($id, $data);
                        $response['id']      = $id;
    ...
    public function View_Product(){
        if ($this->session->userdata('user_login_access') != False) {
            $data                  = array();
            $data['settingsvalue'] = $this->mech_model->getSettingsValue();
            $data['category']      = $this->mech_model->getCategory();
            $data['color']         = $this->mech_model->getColor();
            $data['size']          = $this->mech_model->getSize();
            $data['brand']         = $this->mech_model->getBrand();
            $this->load->view('mech/product', $data);
        } else {
            redirect(base_url(), 'refresh');
        }
    }
    ...
    public function Getprodatabyid(){
        if ($this->session->userdata('user_login_access') != False) {
            ...
            $data['size']          = $this->mech_model->getSize();
            $data['brand']         = $this->mech_model->getBrand();
            $data['productvalue']  = $this->mech_model->getproductdetails($proid);
            $data['productimage']  = $this->mech_model->getproductImages($proid);
            $data['productcolor']  = $this->mech_model->getProductColors($proid);
            $data['productsize']   = $this->mech_model->getProductSizes($proid);
            $this->load->view('mech/update_product', $data);
        } else {
            redirect(base_url(), 'refresh');
        }
    }
    ...
    public function product_list(){
        if ($this->session->userdata('user_login_access') != False) {
            ...
            $data['size']          = $this->mech_model->getSize();
            $data['brand']         = $this->mech_model->getBrand();
            $data['proimage']      = $this->mech_model->getProImage();
            $data['productlist']   = $this->mech_model->getProductData();   
            $this->load->view('mech/productlist', $data);
        } else {
            redirect(base_url(), 'refresh');
        }
    }
    ...
    public function product_details(){
        if ($this->session->userdata('user_login_access') != False) {
            ...
            $data                   = array();
            $data['settingsvalue']  = $this->mech_model->getSettingsValue();
            $data['productdetails'] = $this->mech_model->getproductdetails($proid);
            $data['productsize']    = $this->mech_model->getproductsize($proid);
            $data['productcolor']   = $this->mech_model->getproductcolor($proid);
            $data['productimage']   = $this->mech_model->getproductImage($proid);
            $this->load->view('mech/productdetails', $data);
        } else {
            redirect(base_url(), 'refresh');
        }
    }
    ...
    public function addCategoryData(){
        if ($this->session->userdata('user_login_access') != False) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if (!empty($catid)) {
                    $update              = $this->mech_model->updateCategory($catid, $data);
                    $response['status']  = 'success';
                    $response['message'] = "Successfully Updated";
                    $this->output->set_output(json_encode($response));
                } else {
                    $insert              = $this->mech_model->insertcategory($data);
                    $response['status']  = 'success';
                    $response['message'] = "Successfully Added";
                    $this->output->set_output(json_encode($response));
                }
    ...
    public function addSubCategoryData(){
        if ($this->session->userdata('user_login_access') != False) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if (!empty($id)) {
                    $update              = $this->mech_model->updateSubcategory($id, $data);
                    $response['status']  = 'success';
                    $response['message'] = "Successfully Updated";
                    $this->output->set_output(json_encode($response));
                } else {
                    $insert              = $this->mech_model->insertSubcategory($data);
                    $response['status']  = 'success';
                    $response['message'] = "Successfully Added";
                    $this->output->set_output(json_encode($response));
                }
    ...
    public function getCategoryByID(){
        if ($this->session->userdata('user_login_access') != False) {
            ...
            $subcatlist = $this->mech_model->getsubcategoryByID($catid);
    ...
    public function getSubcategoryByid(){
        if ($this->session->userdata('user_login_access') != False) {
            $id             = $this->input->get('id');
            $data['subcat'] = $this->mech_model->getSubCatById($id);
            echo json_encode($data);
        } else {
            redirect(base_url(), 'refresh');
        }
    ...
    public function getColorById() {
        if ($this->session->userdata('user_login_access') != False) {
            $id                 = $this->input->get('id');
            $data['colorvalue'] = $this->mech_model->getColorById($id);
            echo json_encode($data);
        } else {
            redirect(base_url(), 'refresh');
        }
    ...
    public function getSizeById(){
        if ($this->session->userdata('user_login_access') != False) {
            $id                = $this->input->get('id');
            $data['sizevalue'] = $this->mech_model->getSizeBYId($id);
            echo json_encode($data);
        } else {
            redirect(base_url(), 'refresh');
        }
    ...
    public function getBrandById(){
        if ($this->session->userdata('user_login_access') != False) {
            $id                 = $this->input->get('id');
            $data['brandvalue'] = $this->mech_model->getBrandBYID($id);
            echo json_encode($data);
        } else {
            redirect(base_url(), 'refresh');
        }
    ...
    public function addSizeData(){
        if ($this->session->userdata('user_login_access') != False) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if (!empty($id)) {
                    $update              = $this->mech_model->updateSizeValue($id, $data);
                    $response['status']  = 'success';
                    $response['message'] = "Successfully Updated";
                    $this->output->set_output(json_encode($response));
                } else {
                    $insert              = $this->mech_model->insertSizeValue($data);
                    $response['status']  = 'success';
                    $response['message'] = "Successfully Added";
                    $this->output->set_output(json_encode($response));
                }
    ...
    public function addColorData(){
        if ($this->session->userdata('user_login_access') != False) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if (!empty($id)) {
                    $update              = $this->mech_model->updateColorValue($id, $data);
                    $response['status']  = 'success';
                    $response['message'] = "Successfully Updated";
                    $this->output->set_output(json_encode($response));
                } else {
                    $insert              = $this->mech_model->insertColorValue($data);
                    $response['status']  = 'success';
                    $response['message'] = "Successfully Added";
                    $this->output->set_output(json_encode($response));
                }
    ...
    public function addBrandData(){
        if ($this->session->userdata('user_login_access') != False) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if (!empty($id)) {
                    $update              = $this->mech_model->updateBrandValue($id, $data);
                    $response['status']  = 'success';
                    $response['message'] = "Successfully Updated";
                    $this->output->set_output(json_encode($response));
                } else {
                    $insert              = $this->mech_model->insertBrandValue($data);
                    $response['status']  = 'success';
                    $response['message'] = "Successfully Added";
                    $this->output->set_output(json_encode($response));
                }
