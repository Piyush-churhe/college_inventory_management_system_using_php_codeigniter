...
class Electrical extends CI_Controller{
    ...
    public function dashboard(){
        if ($this->session->userdata('user_login_access') != False) {
            ...
            $data['todolist'] = $this->electrical_model->getTodoInfo($userid);
            $this->load->view('electrical/dashboard', $data);
        } else {
    ...
    public function List_user(){
        if ($this->session->userdata('user_login_access') != False) {
            $data                  = array();
            $data['userlist']      = $this->electrical_model->getAllUsers();
            $data['settingsvalue'] = $this->electrical_model->getSettingsValue();
            $this->load->view('electrical/userlist', $data);
        } else {
            redirect(base_url(), 'refresh');
        }
    }
    ...
    public function List_user_updated(){
        if ($this->session->userdata('user_login_access') != False) {
            ...
            $data['userlist']      = $this->electrical_model->getAllUsers();
            $data['settingsvalue'] = $this->electrical_model->getSettingsValue();
            $this->load->view('electrical/userlist-updated', $data);
        } else {
            redirect(base_url(), 'refresh');
        }
    }
    ...
    public function Add_User(){
        if ($this->session->userdata('user_login_access') != False) {
            $data['settingsvalue'] = $this->electrical_model->getSettingsValue();
            $this->load->view('electrical/adduser', $data);
        } else {
            redirect(base_url(), 'refresh');
        }
    }
    ...
    public function user_delet(){
        if ($this->session->userdata('user_login_access') != False) {
            ...
            $this->electrical_model->userTableDelet($id);
            if ($this->db->affected_rows()) {
                $profile    = $this->electrical_model->getUserValue($id);
                $checkimage = "./assets/img/user/$profile->image";
                if (file_exists($checkimage)) {
                    unlink($checkimage);
                    redirect('electrical/List_user');
                }
                /*      $this->electrical_model->User_Notes_Delet($id);
                $this->electrical_model->User_commentid_Delet($id);*/
                
            } else {
                redirect('electrical/List_user', 'refresh');
            }
        }
    }
    /*user profile*/
    public function View_profile(){
        if ($this->session->userdata('user_login_access') != False) {
            ...
            $data                  = array();
            $data['settingsvalue'] = $this->electrical_model->getSettingsValue();
            $data['profile']       = $this->electrical_model->getProfileValue($userid);
            $data['usernote']      = $this->electrical_model->getUserNotes($userid);
            $this->load->view('electrical/profile', $data);
        } else {
            redirect(base_url(), 'refresh');
        }
    }
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                for ($i = 0; $i < $cpt; $i++) {
                    ...
                    if ($this->upload->do_upload('product_image')) {
                        ...
                        $success                     = $this->electrical_model->productImgInsert($data1);
                    }
                }
                if (!empty($uploadData)) {
                    $data                = array();
                    ...
                    $success             = $this->electrical_model->productInsert($data);
                    ...
                    if ($insertid) {
                        ...
                        foreach ($color as $colorvalue) {
                            ...
                            $success     = $this->electrical_model->productColor($data);
                        ...
                        foreach ($size as $sizevalue) {
                            ...
                            $success       = $this->electrical_model->productSize($data);
        ...
        if ($this->session->userdata('user_login_access') != False) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if (!empty($id)) {
                    ...
                    $success    = $this->electrical_model->productUpdateInfo($id, $data);
                    $deletcolor = $this->electrical_model->delet_Color($id);
                    $deletsize  = $this->electrical_model->delet_Size($id);
                    $color      = $this->input->post('color[]');
                    $size       = $this->input->post('size[]');
                    foreach ($color as $colorvalue) {
                        $data        = array();
    ...
    public function viewProDataBYID(){
        if ($this->session->userdata('user_login_access') != False) {
            $id                = $this->input->get('id');
            $data['uservalue'] = $this->electrical_model->getProValue($id);
    ...
    public function viewUserDataBYID(){
        if ($this->session->userdata('user_login_access') != False) {
            $id                = $this->input->get('id');
            $data['uservalue'] = $this->electrical_model->getUserValue($id);
            echo json_encode($data);
        } else {
            redirect(base_url(), 'refresh');
        }
                ...
                if ($this->login_model->Does_email_exists($email)) {
                    ...
                    if ($_FILES['user_image']['name']) {
                        ...
                        if (!$this->upload->do_upload('user_image')) {
                            ...
                            $success             = $this->electrical_model->addUserInfo($data);
                            $response['status']  = 'success';
                            $response['message'] = "Successfully Added";
                            $this->output->set_output(json_encode($response));
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if ($_FILES['user_image']['name']) {
                    ...
                    if (!$this->upload->do_upload('user_image')) {
                        ...
                        $success             = $this->electrical_model->UserUpdate($id, $data);
                        $response['id']      = $id;
    ...
    public function View_Product(){
        if ($this->session->userdata('user_login_access') != False) {
            $data                  = array();
            $data['settingsvalue'] = $this->electrical_model->getSettingsValue();
            $data['category']      = $this->electrical_model->getCategory();
            $data['color']         = $this->electrical_model->getColor();
            $data['size']          = $this->electrical_model->getSize();
            $data['brand']         = $this->electrical_model->getBrand();
            $this->load->view('electrical/product', $data);
        } else {
            redirect(base_url(), 'refresh');
        }
    }
    ...
    public function Getprodatabyid(){
        if ($this->session->userdata('user_login_access') != False) {
            ...
            $data['size']          = $this->electrical_model->getSize();
            $data['brand']         = $this->electrical_model->getBrand();
            $data['productvalue']  = $this->electrical_model->getproductdetails($proid);
            $data['productimage']  = $this->electrical_model->getproductImages($proid);
            $data['productcolor']  = $this->electrical_model->getProductColors($proid);
            $data['productsize']   = $this->electrical_model->getProductSizes($proid);
            $this->load->view('electrical/update_product', $data);
        } else {
            redirect(base_url(), 'refresh');
        }
    }
    ...
    public function product_list(){
        if ($this->session->userdata('user_login_access') != False) {
            ...
            $data['size']          = $this->electrical_model->getSize();
            $data['brand']         = $this->electrical_model->getBrand();
            $data['proimage']      = $this->electrical_model->getProImage();
            $data['productlist']   = $this->electrical_model->getProductData();   
            $this->load->view('electrical/productlist', $data);
        } else {
            redirect(base_url(), 'refresh');
        }
    }
    ...
    public function product_details(){
        if ($this->session->userdata('user_login_access') != False) {
            ...
            $data                   = array();
            $data['settingsvalue']  = $this->electrical_model->getSettingsValue();
            $data['productdetails'] = $this->electrical_model->getproductdetails($proid);
            $data['productsize']    = $this->electrical_model->getproductsize($proid);
            $data['productcolor']   = $this->electrical_model->getproductcolor($proid);
            $data['productimage']   = $this->electrical_model->getproductImage($proid);
            $this->load->view('electrical/productdetails', $data);
        } else {
            redirect(base_url(), 'refresh');
        }
    }
    ...
    public function addCategoryData(){
        if ($this->session->userdata('user_login_access') != False) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if (!empty($catid)) {
                    $update              = $this->electrical_model->updateCategory($catid, $data);
                    $response['status']  = 'success';
                    $response['message'] = "Successfully Updated";
                    $this->output->set_output(json_encode($response));
                } else {
                    $insert              = $this->electrical_model->insertcategory($data);
                    $response['status']  = 'success';
                    $response['message'] = "Successfully Added";
                    $this->output->set_output(json_encode($response));
                }
    ...
    public function addSubCategoryData(){
        if ($this->session->userdata('user_login_access') != False) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if (!empty($id)) {
                    $update              = $this->electrical_model->updateSubcategory($id, $data);
                    $response['status']  = 'success';
                    $response['message'] = "Successfully Updated";
                    $this->output->set_output(json_encode($response));
                } else {
                    $insert              = $this->electrical_model->insertSubcategory($data);
                    $response['status']  = 'success';
                    $response['message'] = "Successfully Added";
                    $this->output->set_output(json_encode($response));
                }
    ...
    public function getCategoryByID(){
        if ($this->session->userdata('user_login_access') != False) {
            ...
            $subcatlist = $this->electrical_model->getsubcategoryByID($catid);
    ...
    public function getSubcategoryByid(){
        if ($this->session->userdata('user_login_access') != False) {
            $id             = $this->input->get('id');
            $data['subcat'] = $this->electrical_model->getSubCatById($id);
            echo json_encode($data);
        } else {
            redirect(base_url(), 'refresh');
        }
    ...
    public function getColorById() {
        if ($this->session->userdata('user_login_access') != False) {
            $id                 = $this->input->get('id');
            $data['colorvalue'] = $this->electrical_model->getColorById($id);
            echo json_encode($data);
        } else {
            redirect(base_url(), 'refresh');
        }
    ...
    public function getSizeById(){
        if ($this->session->userdata('user_login_access') != False) {
            $id                = $this->input->get('id');
            $data['sizevalue'] = $this->electrical_model->getSizeBYId($id);
            echo json_encode($data);
        } else {
            redirect(base_url(), 'refresh');
        }
    ...
    public function getBrandById(){
        if ($this->session->userdata('user_login_access') != False) {
            $id                 = $this->input->get('id');
            $data['brandvalue'] = $this->electrical_model->getBrandBYID($id);
            echo json_encode($data);
        } else {
            redirect(base_url(), 'refresh');
        }
    ...
    public function addSizeData(){
        if ($this->session->userdata('user_login_access') != False) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if (!empty($id)) {
                    $update              = $this->electrical_model->updateSizeValue($id, $data);
                    $response['status']  = 'success';
                    $response['message'] = "Successfully Updated";
                    $this->output->set_output(json_encode($response));
                } else {
                    $insert              = $this->electrical_model->insertSizeValue($data);
                    $response['status']  = 'success';
                    $response['message'] = "Successfully Added";
                    $this->output->set_output(json_encode($response));
                }
    ...
    public function addColorData(){
        if ($this->session->userdata('user_login_access') != False) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if (!empty($id)) {
                    $update              = $this->electrical_model->updateColorValue($id, $data);
                    $response['status']  = 'success';
                    $response['message'] = "Successfully Updated";
                    $this->output->set_output(json_encode($response));
                } else {
                    $insert              = $this->electrical_model->insertColorValue($data);
                    $response['status']  = 'success';
                    $response['message'] = "Successfully Added";
                    $this->output->set_output(json_encode($response));
                }
    ...
    public function addBrandData(){
        if ($this->session->userdata('user_login_access') != False) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if (!empty($id)) {
                    $update              = $this->electrical_model->updateBrandValue($id, $data);
                    $response['status']  = 'success';
                    $response['message'] = "Successfully Updated";
                    $this->output->set_output(json_encode($response));
                } else {
                    $insert              = $this->electrical_model->insertBrandValue($data);
                    $response['status']  = 'success';
                    $response['message'] = "Successfully Added";
                    $this->output->set_output(json_encode($response));
                }
