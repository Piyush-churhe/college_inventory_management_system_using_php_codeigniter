...
class Workshop extends CI_Controller{
    ...
    public function dashboard(){
        if ($this->session->userdata('user_login_access') != False) {
            ...
            $data['todolist'] = $this->workshop_model->getTodoInfo($userid);
            $this->load->view('workshop/dashboard', $data);
        } else {
    ...
    public function List_user(){
        if ($this->session->userdata('user_login_access') != False) {
            $data                  = array();
            $data['userlist']      = $this->workshop_model->getAllUsers();
            $data['settingsvalue'] = $this->workshop_model->getSettingsValue();
    ...
    public function List_user_updated(){
        if ($this->session->userdata('user_login_access') != False) {
            ...
            $data['userlist']      = $this->workshop_model->getAllUsers();
            $data['settingsvalue'] = $this->workshop_model->getSettingsValue();
            $this->load->view('workshop/userlist-updated', $data);
        } else {
            redirect(base_url(), 'refresh');
        }
    }
    ...
    public function Add_User(){
        if ($this->session->userdata('user_login_access') != False) {
            $data['settingsvalue'] = $this->workshop_model->getSettingsValue();
            $this->load->view('workshop/adduser', $data);
        } else {
            redirect(base_url(), 'refresh');
        }
    }
    ...
    public function user_delet(){
        if ($this->session->userdata('user_login_access') != False) {
            ...
            $this->workshop_model->userTableDelet($id);
            if ($this->db->affected_rows()) {
                $profile    = $this->workshop_model->getUserValue($id);
                $checkimage = "./assets/img/user/$profile->image";
                if (file_exists($checkimage)) {
                    unlink($checkimage);
                    redirect('crud/List_user');
                }
                /*      $this->workshop_model->User_Notes_Delet($id);
                $this->workshop_model->User_commentid_Delet($id);*/
    ...
    public function View_profile(){
        if ($this->session->userdata('user_login_access') != False) {
            ...
            $data                  = array();
            $data['settingsvalue'] = $this->workshop_model->getSettingsValue();
            $data['profile']       = $this->workshop_model->getProfileValue($userid);
            $data['usernote']      = $this->workshop_model->getUserNotes($userid);
            $this->load->view('workshop/profile', $data);
        } else {
            redirect(base_url(), 'refresh');
        }
    }
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                for ($i = 0; $i < $cpt; $i++) {
                    ...
                    if ($this->upload->do_upload('product_image')) {
                        ...
                        $success                     = $this->workshop_model->productImgInsert($data1);
                    }
                }
                if (!empty($uploadData)) {
                    $data                = array();
                    ...
                    $success             = $this->workshop_model->productInsert($data);
                    ...
                    if ($insertid) {
                        ...
                        foreach ($color as $colorvalue) {
                            ...
                            $success     = $this->workshop_model->productColor($data);
                        ...
                        foreach ($size as $sizevalue) {
                            ...
                            $success       = $this->workshop_model->productSize($data);
        ...
        if ($this->session->userdata('user_login_access') != False) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if (!empty($id)) {
                    ...
                    $success    = $this->workshop_model->productUpdateInfo($id, $data);
                    $deletcolor = $this->workshop_model->delet_Color($id);
                    $deletsize  = $this->workshop_model->delet_Size($id);
                    $color      = $this->input->post('color[]');
                    $size       = $this->input->post('size[]');
                    foreach ($color as $colorvalue) {
                        $data        = array();
    ...
    public function viewUserDataBYID(){
        if ($this->session->userdata('user_login_access') != False) {
            $id                = $this->input->get('id');
            $data['uservalue'] = $this->workshop_model->getUserValue($id);
                ...
                if ($this->login_model->Does_email_exists($email)) {
                    ...
                    if ($_FILES['user_image']['name']) {
                        ...
                        if (!$this->upload->do_upload('user_image')) {
                            ...
                            $success             = $this->workshop_model->addUserInfo($data);
                            $response['status']  = 'success';
                            $response['message'] = "Successfully Added";
                            $this->output->set_output(json_encode($response));
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if ($_FILES['user_image']['name']) {
                    ...
                    if (!$this->upload->do_upload('user_image')) {
                        ...
                        $success             = $this->workshop_model->UserUpdate($id, $data);
                        $response['id']      = $id;
    ...
    public function View_Product(){
        if ($this->session->userdata('user_login_access') != False) {
            $data                  = array();
            $data['settingsvalue'] = $this->workshop_model->getSettingsValue();
            $data['category']      = $this->workshop_model->getCategory();
            $this->load->view('workshop/product', $data);
        } else {
            redirect(base_url(), 'refresh');
        }
    }
    ...
    public function Getprodatabyid(){
        if ($this->session->userdata('user_login_access') != False) {
            ...
            $data['settingsvalue'] = $this->workshop_model->getSettingsValue();
            $data['category']      = $this->workshop_model->getCategory();
            $data['productvalue']  = $this->workshop_model->getproductdetails($proid);
            $this->load->view('workshop/update_product', $data);
        } else {
            redirect(base_url(), 'refresh');
        }
    }
    ...
    public function product_list(){
        if ($this->session->userdata('user_login_access') != False) {
            ...
            $data['settingsvalue'] = $this->workshop_model->getSettingsValue();
            $data['category']      = $this->workshop_model->getCategory();
            $data['productlist']   = $this->workshop_model->getProductData();   
            $this->load->view('workshop/productlist', $data);
        } else {
            redirect(base_url(), 'refresh');
        }
    }
    ...
    public function product_details(){
        if ($this->session->userdata('user_login_access') != False) {
            ...
            $data                   = array();
            $data['settingsvalue']  = $this->workshop_model->getSettingsValue();
            $data['productdetails'] = $this->workshop_model->getproductdetails($proid);
            $this->load->view('workshop/productdetails', $data);
        } else {
            redirect(base_url(), 'refresh');
        }
    }
    ...
    public function addCategoryData(){
        if ($this->session->userdata('user_login_access') != False) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if (!empty($catid)) {
                    $update              = $this->workshop_model->updateCategory($catid, $data);
                    $response['status']  = 'success';
                    $response['message'] = "Successfully Updated";
                    $this->output->set_output(json_encode($response));
                } else {
                    $insert              = $this->workshop_model->insertcategory($data);
                    $response['status']  = 'success';
                    $response['message'] = "Successfully Added";
                    $this->output->set_output(json_encode($response));
                }
    ...
    public function addSubCategoryData(){
        if ($this->session->userdata('user_login_access') != False) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if (!empty($id)) {
                    $update              = $this->workshop_model->updateSubcategory($id, $data);
                    $response['status']  = 'success';
                    $response['message'] = "Successfully Updated";
                    $this->output->set_output(json_encode($response));
                } else {
                    $insert              = $this->workshop_model->insertSubcategory($data);
                    $response['status']  = 'success';
                    $response['message'] = "Successfully Added";
                    $this->output->set_output(json_encode($response));
                }
    ...
    public function getCategoryByID(){
        if ($this->session->userdata('user_login_access') != False) {
            ...
            $subcatlist = $this->workshop_model->getsubcategoryByID($catid);
    ...
    public function getSubcategoryByid(){
        if ($this->session->userdata('user_login_access') != False) {
            $id             = $this->input->get('id');
            $data['subcat'] = $this->workshop_model->getSubCatById($id);
            echo json_encode($data);
        } else {
            redirect(base_url(), 'refresh');
        }
    ...
    public function getBrandById(){
        if ($this->session->userdata('user_login_access') != False) {
            $id                 = $this->input->get('id');
            $data['brandvalue'] = $this->workshop_model->getBrandBYID($id);
            echo json_encode($data);
        } else {
            redirect(base_url(), 'refresh');
        }
    ...
    public function addSizeData(){
        if ($this->session->userdata('user_login_access') != False) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if (!empty($id)) {
                    $update              = $this->workshop_model->updateSizeValue($id, $data);
                    $response['status']  = 'success';
                    $response['message'] = "Successfully Updated";
                    $this->output->set_output(json_encode($response));
                } else {
                    $insert              = $this->workshop_model->insertSizeValue($data);
                    $response['status']  = 'success';
                    $response['message'] = "Successfully Added";
                    $this->output->set_output(json_encode($response));
                }
    ...
    public function addColorData(){
        if ($this->session->userdata('user_login_access') != False) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if (!empty($id)) {
                    $update              = $this->workshop_model->updateColorValue($id, $data);
                    $response['status']  = 'success';
                    $response['message'] = "Successfully Updated";
                    $this->output->set_output(json_encode($response));
                } else {
                    $insert              = $this->workshop_model->insertColorValue($data);
                    $response['status']  = 'success';
                    $response['message'] = "Successfully Added";
                    $this->output->set_output(json_encode($response));
                }
    ...
    public function addBrandData(){
        if ($this->session->userdata('user_login_access') != False) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if (!empty($id)) {
                    $update              = $this->workshop_model->updateBrandValue($id, $data);
                    $response['status']  = 'success';
                    $response['message'] = "Successfully Updated";
                    $this->output->set_output(json_encode($response));
                } else {
                    $insert              = $this->workshop_model->insertBrandValue($data);
                    $response['status']  = 'success';
                    $response['message'] = "Successfully Added";
                    $this->output->set_output(json_encode($response));
                }
      ...
      public function viewProDataBYID(){
        if ($this->session->userdata('user_login_access') != False) {
            $id                = $this->input->get('id');
            $data['uservalue'] = $this->workshop_model->getProValue($id);
            echo json_encode($data);
        } else {
            redirect(base_url(), 'refresh');
        }
    ...
    public function updateProValue(){
        if ($this->session->userdata('user_login_access') != False) {
            ...
            $WS_Approval    = $this->input->post('WS_Approval');
            $this->load->library('form_validation');
            $this->form_validation->set_error_delimiters();
            /*validating gender field*/
            $this->form_validation->set_rules('WS_Approval', 'Principal Approval', 'trim|xss_clean');
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $this->output->set_output(json_encode($response));
                    ...
                    $data                = array();
                    $data                = array(
                        'WS_Approval' => $WS_Approval
                       
                    );
                    $success             = $this->workshop_model->ProductUpdate($pro_id, $data);
                    $response['id']      = $id;
                    $response['data']    = $data;
                    $response['status']  = 'success';
                    $response['message'] = "Successfully Updated";
