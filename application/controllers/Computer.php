...
class Computer extends CI_Controller{
    ...
    public function dashboard(){
        if ($this->session->userdata('user_login_access') != False) {
            ...
            $data['todolist'] = $this->computer_model->getTodoInfo($userid);
            $this->load->view('computer/dashboard', $data);
        } else {
    ...
    public function List_user(){
        if ($this->session->userdata('user_login_access') != False) {
            $data                  = array();
            $data['userlist']      = $this->computer_model->getAllUsers();
            $data['settingsvalue'] = $this->computer_model->getSettingsValue();
            $this->load->view('computer/userlist', $data);
        } else {
            redirect(base_url(), 'refresh');
        }
    }
    ...
    public function List_user_updated(){
        if ($this->session->userdata('user_login_access') != False) {
            ...
            $data['userlist']      = $this->computer_model->getAllUsers();
            $data['settingsvalue'] = $this->computer_model->getSettingsValue();
            $this->load->view('computer/userlist-updated', $data);
        } else {
            redirect(base_url(), 'refresh');
        }
    }
    ...
    public function Add_User(){
        if ($this->session->userdata('user_login_access') != False) {
            $data['settingsvalue'] = $this->computer_model->getSettingsValue();
            $this->load->view('computer/adduser', $data);
        } else {
            redirect(base_url(), 'refresh');
        }
    }
    ...
    public function user_delet(){
        if ($this->session->userdata('user_login_access') != False) {
            ...
            $this->computer_model->userTableDelet($id);
            if ($this->db->affected_rows()) {
                $profile    = $this->computer_model->getUserValue($id);
                $checkimage = "./assets/img/user/$profile->image";
                if (file_exists($checkimage)) {
                    unlink($checkimage);
                    redirect('computer/List_user');
                }
                /*      $this->computer_model->User_Notes_Delet($id);
                $this->computer_model->User_commentid_Delet($id);*/
                
            } else {
                redirect('computer/List_user', 'refresh');
            }
        }
    }
    /*user profile*/
    public function View_profile(){
        if ($this->session->userdata('user_login_access') != False) {
            ...
            $data                  = array();
            $data['settingsvalue'] = $this->computer_model->getSettingsValue();
            $data['profile']       = $this->computer_model->getProfileValue($userid);
            $data['usernote']      = $this->computer_model->getUserNotes($userid);
            $this->load->view('computer/profile', $data);
        } else {
            redirect(base_url(), 'refresh');
        }
    }
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                for ($i = 0; $i < $cpt; $i++) {
                    ...
                    if ($this->upload->do_upload('product_image')) {
                        ...
                        $success                     = $this->computer_model->productImgInsert($data1);
                    }
                }
                if (!empty($uploadData)) {
                    $data                = array();
                    ...
                    $success             = $this->computer_model->productInsert($data);
                    ...
                    if ($insertid) {
                        ...
                        foreach ($color as $colorvalue) {
                            ...
                            $success     = $this->computer_model->productColor($data);
                        ...
                        foreach ($size as $sizevalue) {
                            ...
                            $success       = $this->computer_model->productSize($data);
        ...
        if ($this->session->userdata('user_login_access') != False) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if (!empty($id)) {
                    ...
                    $success    = $this->computer_model->productUpdateInfo($id, $data);
                    $deletcolor = $this->computer_model->delet_Color($id);
                    $deletsize  = $this->computer_model->delet_Size($id);
                    $color      = $this->input->post('color[]');
                    $size       = $this->input->post('size[]');
                    foreach ($color as $colorvalue) {
                        $data        = array();
    ...
    public function viewUserDataBYID(){
        if ($this->session->userdata('user_login_access') != False) {
            $id                = $this->input->get('id');
            $data['uservalue'] = $this->computer_model->getUserValue($id);
    ...
    public function addUserInfo(){
                ...
                if ($this->login_model->Does_email_exists($email)) {
                    ...
                    if ($_FILES['user_image']['name']) {
                        ...
                        if (!$this->upload->do_upload('user_image')) {
                            ...
                            $success             = $this->computer_model->addUserInfo($data);
                            $response['status']  = 'success';
                            $response['message'] = "Successfully Added";
                            $this->output->set_output(json_encode($response));
      ...
      public function viewProDataBYID(){
        if ($this->session->userdata('user_login_access') != False) {
            $id                = $this->input->get('id');
            $data['uservalue'] = $this->computer_model->getProValue($id);
            echo json_encode($data);
        } else {
            redirect(base_url(), 'refresh');
        }
    ...
    public function updateProValue(){
        if ($this->session->userdata('user_login_access') != False) {
            ...
            $Hod_Approval    = $this->input->post('Hod_Approval');
            $this->load->library('form_validation');
            $this->form_validation->set_error_delimiters();
            /*validating gender field*/
            $this->form_validation->set_rules('Hod_Approval', 'Hod Approval', 'trim|xss_clean');
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $this->output->set_output(json_encode($response));
                    ...
                    $success             = $this->computer_model->ProductUpdate($pro_id, $data);
                    $response['id']      = $id;
                    $response['data']    = $data;
                    $response['status']  = 'success';
                    $response['message'] = "Successfully Updated";
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if ($_FILES['user_image']['name']) {
                    ...
                    if (!$this->upload->do_upload('user_image')) {
                        ...
                        $success             = $this->computer_model->UserUpdate($id, $data);
                        $response['id']      = $id;
    ...
    public function View_Product(){
        if ($this->session->userdata('user_login_access') != False) {
            $data                  = array();
            $data['settingsvalue'] = $this->computer_model->getSettingsValue();
            $data['category']      = $this->computer_model->getCategory();
            $this->load->view('computer/product', $data);
        } else {
            redirect(base_url(), 'refresh');
        }
    }
    ...
    public function Getprodatabyid(){
        if ($this->session->userdata('user_login_access') != False) {
            ...
            $data['settingsvalue'] = $this->computer_model->getSettingsValue();
            $data['category']      = $this->computer_model->getCategory();
            $data['productvalue']  = $this->computer_model->getproductdetails($proid);
            $this->load->view('computer/update_product', $data);
        } else {
            redirect(base_url(), 'refresh');
        }
    }
    ...
    public function product_list(){
        if ($this->session->userdata('user_login_access') != False) {
            ...
            $data['settingsvalue'] = $this->computer_model->getSettingsValue();
            $data['category']      = $this->computer_model->getCategory();
            $data['productlist']   = $this->computer_model->getProductData();   
            $this->load->view('computer/productlist', $data);
        } else {
            redirect(base_url(), 'refresh');
        }
    }
    ...
    public function product_details(){
        if ($this->session->userdata('user_login_access') != False) {
            ...
            $data                   = array();
            $data['settingsvalue']  = $this->computer_model->getSettingsValue();
            $data['productdetails'] = $this->computer_model->getproductdetails($proid);
            $this->load->view('computer/productdetails', $data);
        } else {
            redirect(base_url(), 'refresh');
        }
    }
    ...
    public function addCategoryData(){
        if ($this->session->userdata('user_login_access') != False) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if (!empty($catid)) {
                    $update              = $this->computer_model->updateCategory($catid, $data);
                    $response['status']  = 'success';
                    $response['message'] = "Successfully Updated";
                    $this->output->set_output(json_encode($response));
                } else {
                    $insert              = $this->computer_model->insertcategory($data);
                    $response['status']  = 'success';
                    $response['message'] = "Successfully Added";
                    $this->output->set_output(json_encode($response));
                }
    ...
    public function addSubCategoryData(){
        if ($this->session->userdata('user_login_access') != False) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if (!empty($id)) {
                    $update              = $this->computer_model->updateSubcategory($id, $data);
                    $response['status']  = 'success';
                    $response['message'] = "Successfully Updated";
                    $this->output->set_output(json_encode($response));
                } else {
                    $insert              = $this->computer_model->insertSubcategory($data);
                    $response['status']  = 'success';
                    $response['message'] = "Successfully Added";
                    $this->output->set_output(json_encode($response));
                }
    ...
    public function getCategoryByID(){
        if ($this->session->userdata('user_login_access') != False) {
            ...
            $subcatlist = $this->computer_model->getsubcategoryByID($catid);
    ...
    public function getSubcategoryByid(){
        if ($this->session->userdata('user_login_access') != False) {
            $id             = $this->input->get('id');
            $data['subcat'] = $this->computer_model->getSubCatById($id);
            echo json_encode($data);
        } else {
            redirect(base_url(), 'refresh');
        }
    ...
    public function getColorById() {
        if ($this->session->userdata('user_login_access') != False) {
            $id                 = $this->input->get('id');
            $data['colorvalue'] = $this->computer_model->getColorById($id);
            echo json_encode($data);
        } else {
            redirect(base_url(), 'refresh');
        }
    ...
    public function getSizeById(){
        if ($this->session->userdata('user_login_access') != False) {
            $id                = $this->input->get('id');
            $data['sizevalue'] = $this->computer_model->getSizeBYId($id);
            echo json_encode($data);
        } else {
            redirect(base_url(), 'refresh');
        }
    ...
    public function getBrandById(){
        if ($this->session->userdata('user_login_access') != False) {
            $id                 = $this->input->get('id');
            $data['brandvalue'] = $this->computer_model->getBrandBYID($id);
            echo json_encode($data);
        } else {
            redirect(base_url(), 'refresh');
        }
    ...
    public function addSizeData(){
        if ($this->session->userdata('user_login_access') != False) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if (!empty($id)) {
                    $update              = $this->computer_model->updateSizeValue($id, $data);
                    $response['status']  = 'success';
                    $response['message'] = "Successfully Updated";
                    $this->output->set_output(json_encode($response));
                } else {
                    $insert              = $this->computer_model->insertSizeValue($data);
                    $response['status']  = 'success';
                    $response['message'] = "Successfully Added";
                    $this->output->set_output(json_encode($response));
                }
    ...
    public function addColorData(){
        if ($this->session->userdata('user_login_access') != False) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if (!empty($id)) {
                    $update              = $this->computer_model->updateColorValue($id, $data);
                    $response['status']  = 'success';
                    $response['message'] = "Successfully Updated";
                    $this->output->set_output(json_encode($response));
                } else {
                    $insert              = $this->computer_model->insertColorValue($data);
                    $response['status']  = 'success';
                    $response['message'] = "Successfully Added";
                    $this->output->set_output(json_encode($response));
                }
    ...
    public function addBrandData(){
        if ($this->session->userdata('user_login_access') != False) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if (!empty($id)) {
                    $update              = $this->computer_model->updateBrandValue($id, $data);
                    $response['status']  = 'success';
                    $response['message'] = "Successfully Updated";
                    $this->output->set_output(json_encode($response));
                } else {
                    $insert              = $this->computer_model->insertBrandValue($data);
                    $response['status']  = 'success';
                    $response['message'] = "Successfully Added";
                    $this->output->set_output(json_encode($response));
                }
