<?php $this->load->view('lecturer/header'); ?>
                                    ...
                                    <table id="example" class="table table-condensed responsive table_custom" cellspacing="0" width="100%">
                                        ...
                                        <?php $sr=1; foreach($productlist as $value): ?>
                                            <tr>
                                                ...
                                                <td><a href="<?php echo base_url();?>lecturer/product_details?P=<?php echo base64_encode($value->pro_id) ?>"><?php echo $value->pro_name ?><a/></td>
...
<button type="button" name="submit" class="userbutton btn btn-default btn-custom" data-id="<?php echo $value->pro_id; ?>">Request</button>
                                               ...
                                               <!--  <td class="action-buttons">
                                                    <a href="<?php echo base_url();?>lecturer/Getprodatabyid?P=<?php echo base64_encode($value->pro_id) ?>">
                                                        <i class="icon-pencil"></i>
                                                    </a>
                                                    <a onclick="confirm('Are you sure want to delet this product?')" href="<?php echo base_url();?>lecturer/Delet_ProductInfo?D=<?php echo base64_encode($value->pro_id) ?>">
            ...
            <form role="form" action="updateValue" id="UserValueUpdate" method="post" enctype="multipart/form-data" accept-charset="utf-8">
                <div class="modal-body">
                    <div class="form-group clearfix">
                        <label for="inputMaxLength" class="col-md-3">Quantity</label>
                        <div class="col-md-9">
                            <input type="text" name="req_quantity" id="req_quantity" class="form-control" id="inputMaxLength" value='' placeholder="Enter Quantity" required>
                        </div>
                    </div>
...
<script type="text/javascript">
    $(document).ready(function() {
        $(".userbutton").click(function(e) {
            ...
            $.ajax({
                url: '<?php echo base_url(); ?>lecturer/viewProDataBYID?id=' + iid,
                method: 'GET',
                data: '',
                dataType: 'json',
    ...
    $(document).ready(function() {
        $("#btnSubmit").click(function(event) {
            ...
            $.ajax({
                ...
                url: "<?php echo base_url(); ?>lecturer/updateProValue",
                data: data,
                dataType: 'json',
                processData: false,
                contentType: false,
                ...
                success: function(response) {
                    if (response.status == 'error') {
                        ...
                        tr.find('td:eq(1)').text(response.data['pro_name']);
...
<?php $this->load->view('lecturer/footer'); ?>